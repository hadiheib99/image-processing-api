...
describe("API Endpoints", () => {
    ...
    it("should return 400 if width is missing", async () => {
        const res = await request(app).get("/api/images?filename=palmtunnel.jpg&height=200");
        expect(res.status).toBe(400);
        expect(res.body.error).toContain("Missing required query parameters");
    });
    it("should return 400 if height is missing", async () => {
        const res = await request(app).get("/api/images?filename=palmtunnel.jpg&width=200");
        expect(res.status).toBe(400);
        expect(res.body.error).toContain("Missing required query parameters");
    });
    it("should process and return the image if all query params are valid", async () => {
        const res = await request(app).get("/api/images?filename=palmtunnel.jpg&width=200&height=200");
        expect(res.status).toBe(200);
        expect(res.headers["content-type"]).toMatch(/image/);
    });
    it("should return 404 and error message if image does not exist", async () => {
        const res = await request(app).get("/api/images?filename=notfound.jpg&width=200&height=200");
